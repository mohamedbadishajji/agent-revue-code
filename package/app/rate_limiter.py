...
def retry_with_backoff(
    max_retries: int = MAX_RETRIES, initial_wait: float = INITIAL_WAIT
):
    """
                ...
                try:
                    ...
                    if is_rate_limit:
                        ...
                        if attempt < max_retries - 1:
                            print(
                                f"   ⚠️ Rate limit détecté — attente {wait_time}s avant réessai ({attempt + 1}/{max_retries})"
                            )
                            time.sleep(wait_time)
                            wait_time = min(wait_time * 2, MAX_WAIT)
                        else:
                            print(
                                f"   ❌ Rate limit — nombre maximum de tentatives atteint ({max_retries})"
                            )
...
def check_rate_limit(github_client) -> dict:
    ...
    try:
        ...
        if remaining < 100:
            ...
            if wait_seconds > 0:
                print(
                    f"   ⚠️ Quota faible ({remaining} restantes) — attente {int(wait_seconds)}s"
                )
...
def batch_post_comments(
    post_func, comments: list, batch_size: int = 5, delay: float = 0.5
) -> int:
    """

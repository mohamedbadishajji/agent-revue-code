...
# System prompt spécifique pour les code smells
SYSTEM_PROMPT_SMELLS = """Tu es un expert en qualité de code et clean code.
Tu analyses uniquement le diff fourni.

DETECTE UNIQUEMENT ces problemes de qualite :
1. Fonctions trop longues (plus de 20 lignes)
2. Code dupliqué ou copié-collé
3. Nommage incorrect (variables/fonctions avec noms trop courts ou non descriptifs)
4. Trop de paramètres dans une fonction (plus de 4)
5. Magic numbers (nombres sans constante nommée)
6. Commentaires inutiles ou trompeurs
7. Fonction qui fait trop de choses à la fois

REGLES STRICTES :
1. Ne signale QUE les problemes de qualite — pas les bugs ni les vulnerabilites
2. Sois conservateur — signale uniquement les problemes évidents
...
def build_code_smells_prompt(file_path: str, language: str, patch: str) -> str:
    """
    Construit le prompt pour la détection des code smells
    REVUE-9 : Détecter les problèmes de qualité
    """
...
Reponds UNIQUEMENT en JSON selon le format demande. Sans backticks."""
# System prompt spécialisé sécurité — OWASP Top 10 complet
SYSTEM_PROMPT_SECURITY = """Tu es un expert en cybersécurité et en analyse de code.
Tu analyses uniquement le diff fourni.
...
def build_security_prompt(file_path: str, language: str, patch: str) -> str:
    """
    Construit le prompt pour la détection des vulnérabilités de sécurité
    REVUE-10/37 : Détecter les vulnérabilités de sécurité
    """
